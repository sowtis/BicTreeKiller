...
public final class AntiBan {
	...
	/**
	 * Number of timed actions, to paint
	 */
	public static int abcActions = 0;
	 ...
	 * 
	 * public static boolean hoverObject(RSObject object) { if (!should_hover)
	 * return false; if (!Entities07.isHovering(object.getModel()) &&
	 * Clicking.hover(object)) { if (print_debug) { debug("Hovering object"); }
	 * return Timing07.waitCondition(() ->
	 * Entities07.isHovering(object.getModel()), General.random(2000, 3000)); }
	 * return false; }
	 */
	 ...
	 */
	/*
	 * public static void hoverNextNPC() { if (!should_hover) return;
	 * 
	 * final RSCharacter interacting =
	 * Player.getRSPlayer().getInteractingCharacter();
	 * 
	 * if (interacting != null) { final String name = interacting.getName(); if
	 * (name != null) { final RSNPC[] next = NPCs.getAll(new Filter<RSNPC>() {
	 * 
	 * @Override public boolean accept(RSNPC npc) { if (npc == null) return
	 * false; RSNPCDefinition def = npc.getDefinition(); if (def == null) return
	 * false; String def_name = def.getName(); if (def_name == null ||
	 * !def_name.equals(name)) return false; return npc.isOnScreen() &&
	 * npc.isClickable() &&
	 * !npc.getPosition().equals(interacting.getPosition()); } }); if (next !=
	 * null) if (Mouse.isInBounds() && Clicking.hover(next))
	 * Timing.waitCondition(Clicking.hover(next), 500); // if
	 * (!Entities07.isHovering(next.getModel()) && Clicking.hover(next)) //
